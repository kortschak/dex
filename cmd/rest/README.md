...
`rest` provides a mechanism that allows JSON RPC-2.0 calls to be forwarded as if they were being made by other modules/services. Permissions for this are controlled by the kernel's `allow_forward` configuration. By default, no forwarding from one module/service to another is allowed; specific forwarding or a global option to allow any forwarding can be controlled through this configuration.
